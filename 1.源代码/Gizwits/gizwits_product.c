...
int8_t gizwitsEventProcess(eventInfo_t *info, uint8_t *gizdata, uint32_t len)
{
    ...
    for(i=0; i<info->num; i++)
    {
        ...
        case EVENT_Humidifier:
            ...
            break;
        case EVENT_Window:
            currentDataPoint.valueWindow = dataPointPtr->valueWindow;
            GIZWITS_LOG("Evt: EVENT_Window %d \n", currentDataPoint.valueWindow);
            if(0x01 == currentDataPoint.valueWindow)
            {
				if(Model_Flag == 0)
				{
					Window_Flag_ON = 1;
...
void userHandle(void)
{
    ...
    currentDataPoint.valueBuzzer = Buzzer_Flag;//Add Sensor Data Collection
    ...
    currentDataPoint.valueSmog = ppm;//Add Sensor Data Collection
