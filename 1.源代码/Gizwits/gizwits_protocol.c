...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagModel)
    {
        bitFieldBitLen += Model_LEN;
    }
    if(0x01 == aFlag->flagFan)
    {
        bitFieldBitLen += Fan_LEN;
    }
    if(0x01 == aFlag->flagHumidifier)
    {
        bitFieldBitLen += Humidifier_LEN;
    }
    if(0x01 == aFlag->flagWindow)
    {
        bitFieldBitLen += Window_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagModel)
    {
        info->event[info->num] = EVENT_Model;
        info->num++;
        dataPoints->valueModel = gizVarlenDecompressionValue(bitFieldOffset,Model_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += Model_LEN;
    }
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagHumidifier)
    {
        info->event[info->num] = EVENT_Humidifier;
        info->num++;
        dataPoints->valueHumidifier = gizVarlenDecompressionValue(bitFieldOffset,Humidifier_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += Humidifier_LEN;
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagWindow)
    {
        info->event[info->num] = EVENT_Window;
        info->num++;
        dataPoints->valueWindow = gizVarlenDecompressionValue(bitFieldOffset,Window_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += Window_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueModel != cur->valueModel)
    {
        GIZWITS_LOG("valueModel Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagModel = 1;
        ret = 1;
    }
    ...
    if(last->valueHumidifier != cur->valueHumidifier)
    {
        GIZWITS_LOG("valueHumidifier Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagHumidifier = 1;
        ret = 1;
    }
    if(last->valueWindow != cur->valueWindow)
    {
        GIZWITS_LOG("valueWindow Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagWindow = 1;
        ret = 1;
    }
    ...
    if(last->valueHumidity != cur->valueHumidity)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            ...
            gizwitsProtocol.waitReportDatapointFlag.flagHumidity = 1;
            ret = 1;
        }
    }
    if(last->valueSmog != cur->valueSmog)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueSmog Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagSmog = 1;
            ret = 1;
        }
    }
    if(last->valuePM25 != cur->valuePM25)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            ...
            gizwitsProtocol.waitReportDatapointFlag.flagPM25 = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagModel)
    {
        gizVarlenCompressValue(bitFieldOffset,Model_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueModel);
        bitFieldOffset += Model_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagHumidifier)
    {
        gizVarlenCompressValue(bitFieldOffset,Humidifier_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueHumidifier);
        bitFieldOffset += Humidifier_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagWindow)
    {
        gizVarlenCompressValue(bitFieldOffset,Window_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueWindow);
        bitFieldOffset += Window_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagHumidity)
    {
        devStatusTmp.valueHumidity = gizY2X(Humidity_RATIO,  Humidity_ADDITION, dataPoints->valueHumidity); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueHumidity,Humidity_LEN);
        byteFieldOffset += Humidity_LEN;
    }

    if(gizwitsProtocol.waitReportDatapointFlag.flagSmog)
    {
        devStatusTmp.valueSmog = exchangeBytes(gizY2X(Smog_RATIO,  Smog_ADDITION, dataPoints->valueSmog)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueSmog,Smog_LEN);
        byteFieldOffset += Smog_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagPM25)
    {
        devStatusTmp.valuePM25 = exchangeBytes(gizY2X(PM25_RATIO,  PM25_ADDITION, dataPoints->valuePM25)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuePM25,PM25_LEN);
        byteFieldOffset += PM25_LEN;
    }
